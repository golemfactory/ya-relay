...
BLUE='\033[0;34m'
# This script is used to test a network with two clients being on two separate networks without exposed ports
...
export EXPOSED_CONTAINER_ID=$(docker ps | grep exposed | awk '{print $1}')
export HIDDEN_CONTAINER_ID=$(docker ps | grep hidden | awk '{print $1}')

export EXPOSED_CLIENT_IP=$(docker inspect -f '{{range.NetworkSettings.Networks}}{{.IPAddress}}{{end}}' $EXPOSED_CONTAINER_ID)
export HIDDEN_CLIENT_IP=$(docker inspect -f '{{range.NetworkSettings.Networks}}{{.IPAddress}}{{end}}' $HIDDEN_CONTAINER_ID)
