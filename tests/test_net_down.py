...
class Jobs(Thread):
    ...
    def __init__(self, ping_jobs: List[Thread], ping_jobs_started: Event):
...
def wait_for_events(events: List[Event], timeout: int):
...
def test_net_down(compose_up):
    ...
    before_tr_events: List[Event] = []
    after_tr_events: List[Event] = []
    tr_jobs: List[Thread] = []
    for i in range(0, 3):
        before_tr = Event()
        after_tr = Event()
    ...
    tr_jobs_thread: Jobs = Jobs(tr_jobs, tr_jobs_started)

    before_ping_events: List[Event] = []
    after_ping_events: List[Event] = []
    ping_jobs: List[Thread] = []
    for i in range(0, 30):
        before_ping = Event()
        after_ping = Event()
    ...
    ping_jobs_thread: Jobs = Jobs(ping_jobs, ping_jobs_started)

    LOGGER.info("Starting ping jobs and transfer job.")
    ping_jobs_thread.start()
    tr_jobs_thread.start()

    wait_for_events(before_tr_events, 10)
    wait_for_events(before_ping_events, 10)
    ...
    wait_for_events(after_tr_events, 10)
    wait_for_events(after_ping_events, 10)
