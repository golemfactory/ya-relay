...
impl Server {
    pub async fn dispatch(&self, from: SocketAddr, packet: PacketKind) -> ServerResult<()> {
        ...
        if !session_id.is_empty() {
            let id = SessionId::try_from(session_id)
                .map_err(|_| Unauthorized::InvalidSessionId(session_id.into()))?;
    ...
    pub async fn run(self) -> anyhow::Result<()> {
        ...
        const DISPATCH_TASK_COUNT: usize = 1;
        ...
        input
            .map(|(packet, addr, timestamp)| {
                ...
                let session_id = PacketKind::session_id(&packet).to_vec();
                let start = Utc::now();
                let pk_type = match &packet {
                    PacketKind::Forward(_) => "forward",
                    PacketKind::Packet(p) => match &p.kind {
                        None => "None",
                        Some(ya_relay_proto::proto::packet::Kind::Control(_)) => "Control",
                        Some(ya_relay_proto::proto::packet::Kind::Request(r)) => match r.kind {
                            Some(Kind::Session(_)) => "Session",
                            Some(Kind::Register(_)) => "Register",
                            Some(Kind::Node(_)) => "Node",
                            Some(Kind::Slot(_)) => "Slot",
                            Some(Kind::Neighbours(_)) => "Neighbours",
                            Some(Kind::ReverseConnection(_)) => "ReverseConnection",
                            Some(Kind::Ping(_)) => "Ping",
                            _ => "r_"
                        },
                        _ => "_"
                    }
                ...
                let fut = async move {
                    ...
                    if let Err(error) = server.dispatch(addr, packet).await {
                        ...
                        if let Some(request_id) = request_id {
                            server
                                .error_response(request_id, session_id.into(), &addr, error)
                                .await;
                        }
                ...
                tokio::time::timeout(DISPATCH_TIMEOUT, fut).inspect_err(move |_| {
                    ...
                    log::error!("Packet dispatch timed out (request={request_id:?}, from={addr}) start={start} {pk_type}");
                })
            })
