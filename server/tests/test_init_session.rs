...
async fn test_query_self_node_info() -> anyhow::Result<()> {
    ...
    let result_endpoints = client
        .register_endpoints_with_session(session, vec![])
        .await
        .unwrap();
    let node_info = client
        .find_node_with_session(session, node_id)
        .await
        .unwrap();
...
async fn test_request_with_invalid_session() -> anyhow::Result<()> {
    ...
    let node_id = client.id().await;
    let session = client.session_id().await.unwrap();
    ...
    let result = client.find_node_with_session(session, node_id).await;
...
async fn test_query_other_node_info() -> anyhow::Result<()> {
    ...
    let node2_info = client1.find_node(node2_id).await.unwrap();
    let node1_info = client2.find_node(node1_id).await.unwrap();
