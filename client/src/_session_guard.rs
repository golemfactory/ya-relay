...
impl GuardedSessions {
    ...
    pub async fn temporary_session(&self, addr: SocketAddr, sink: OutStream) -> Arc<SystemSession> {
        let mut state = self.state.write().await;
        match state.tmp_sessions.get(&addr) {
            None => {
                let session = SystemSession::new(addr, SessionId::generate(), sink);
...
impl SessionInitState {
    pub fn transition(&mut self, new_state: SessionInitState) -> Result<(), SessionError> {
        // return match (&self, new_state) {
        //     (SessionInitState::Retryable, SessionInitState::Initializing) => Ok(()),
        //     (SessionInitState::Initializing, SessionInitState::WaitingForReverseConnection) => {
        //         Ok(())
        //     }
        //     (SessionInitState::Initializing, SessionInitState::ChallengeSent) => Ok(()),
        //     (SessionInitState::ChallengeSent, SessionInitState::ChallengeReceived) => Ok(()),
        //     (SessionInitState::ChallengeReceived, SessionInitState::ChallengeVerified) => Ok(()),
        //     // TODO: Rest
        //     _ => Err(SessionError {}),
        // };
        unimplemented!()
...
struct SessionTarget {
    ...
    state: Arc<RwLock<SessionInitState>>,
    ...
    notify_msg: Arc<broadcast::Sender<SessionInitState>>,
}
...
impl SessionTarget {
    fn new(node_id: NodeId, addresses: Vec<SocketAddr>) -> Self {
        ...
        Self {
            ...
            state: Arc::new(RwLock::new(SessionInitState::Initializing)),
        }
    }
}
