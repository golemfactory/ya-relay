...
use crate::_session::{SessionResult, SystemSession};
...
impl StartingSessions {
    pub fn new(layer: SessionLayer, guarded: GuardedSessions, sink: OutStream) -> StartingSessions {
        StartingSessions {
    ...
    pub async fn temporary_session(&mut self, addr: SocketAddr) -> Arc<SystemSession> {
        self.guarded
        ...
        let init_future = Abortable::new(
            timeout(self.layer.config.incoming_session_timeout, async move {
                ...
                if this.guarded.try_access_unguarded(remote_id).await {
                    log::debug!(
                        "Node [{remote_id}] enters unguarded session initialization."
                    );
    ...
    async fn init_session_handler(
        ...
        // TODO: Reconsider. This probably is not necessary
        ...
        tmp_session
            .send(challenge)
            .await
            .map_err(|_| SessionError::Network("Failed to send challenge".to_string()))?;
        ...
        if let Some((request_id, session)) = rc.next().await {
            ...
            let (node_id, identities) =
                ...
                .map_err(|e| SessionError::BadResponse(format!("Invalid challenge: {e}")))?;
            ...
            let direct_session = self
                .layer
                .register_session(with, session_id, node_id, identities)
                .await
                .map_err(|e| {
                    SessionError::Internal(format!("Failed to add incoming session. {e}"))
                })?;

            // Temporarily pause forwarding from this node
            let session = direct_session.session.clone();
            ...
            session
                ...
                .await
                .map_err(|_| {
                    SessionError::Network("Failed to send challenge response.".to_string())
                })?;
